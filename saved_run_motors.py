...
def motorInitialize(device_port="COM4", protocol_version=2.0):
...
def moveMotorTo(motor_id, pos, portHandler, packetHandler):
    result, error = writeToAddr(ADDR_PRO_GOAL_POSITION, pos, motor_id, 4, portHandler, packetHandler)
    ...
    while 1:
        ...
        pres_pos, result, error = readAddr(ADDR_PRO_PRESENT_POSITION, motor_id, 4, portHandler, packetHandler2)
        if result != COMM_SUCCESS:
            print("%s" % packetHandler2.getTxRxResult(result))
        elif error != 0:
            print("%s" % packetHandler2.getRxPacketError(error))
...
portHandler, packetHandler2 = motorInitialize()
...
while 1:
    ...
    moveMotorTo(DXL2_ID, 100, portHandler, packetHandler2)
